  ...
  return (
    <div className="w-full">
      ...
      <div className="rounded-xl p-4 mb-6" style={{ background: "#0d0d18", border: "1px solid #1e1e2a" }}>
        <h2 className="text-lg font-black text-white mb-3">🎯 Crypto Scalp Scanner (Phase 1 — Paper Only)</h2>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {results.map((r) => (
            <CryptoScalpCard key={r.symbol} result={r} />
          ))}
        </div>
      )}
    </div>
...
function CryptoScalpCard({ result }: { result: CryptoScalpResult }) {
